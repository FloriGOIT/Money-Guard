...
const ListCardsAll = ({
  info,
  isYearMonthForFilter,
  handleAddCardModal,
  handleDeleteCard,
  handleIdForCardUpdate,
}) => {
  const filteredInfoOnPeriod = info
    .filter(el => Number(el.year) === Number(isYearMonthForFilter.year))
    .filter(el => {
      if (isYearMonthForFilter.month === '-') {
        return el;
      } else {
        return el.month === isYearMonthForFilter.month;
      }
    });
      ...
      <table>
        ...
        <tbody>
          {filteredInfoOnPeriod.map(card => {
            const locationCard = `/card/${card.idFrontend}`;
            return (
              <tr key={card.idFrontend}>
                <td className={style.cardDate} style={{ width: '100px' }}>
                  {card.date}
                </td>
                <td className={style.cardType}>
                  {card.type ? (
                    <ImPlus className={style.imPlus} />
                  ) : (
                    <ImMinus className={style.imMinus} />
                  )}
                </td>
                <td className={style.cardCategory} style={{ width: '100px' }}>
                  {card.category}
                </td>
                <td className={style.cardDetails}>{card.details}</td>
                <td className={style.cardAmount} style={{ width: '80px' }}>
                ...
                <td
                  ...
                  onClick={() => handleDeleteCard(card.idFrontend)}
                >
                ...
                <td
                  ...
                  style={{ padding: '0px 5px 5px 5px' }}
                  onClick={() => {
                    handleIdForCardUpdate(card.idFrontend);
                    handleAddCardModal();
                  }}
                >
